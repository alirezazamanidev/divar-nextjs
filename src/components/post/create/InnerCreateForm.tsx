...
export default function InnerCreatePostForm({ category, values, setFieldValue, errors, touched, isSubmitting }: props) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: string, type: 'image' | 'video') => {
    if (e.target.files) {
      ...
      if (totalFiles.length > maxFiles) {
        alert(`شما می‌توانید حداکثر ${maxFiles} ${type === 'image' ? 'تصویر' : 'ویدیو'} آپلود کنید`);
        return;
      }
  ...
  const removeFile = (index: number, fieldName: string, type: 'image' | 'video') => {
  ...
  return (
    <Form className="w-full max-w-3xl mx-auto p-6 md:p-8 bg-gray-900 rounded-xl shadow-xl text-gray-100">
      ...
      <div className="mb-8 border-t border-gray-800 pt-6">
        <h3 className="text-lg font-medium mb-4 text-right text-gray-300">رسانه‌ها</h3>
        
        {/* Images Upload */}
        <div className="mb-6">
          <label className="block text-gray-200 text-sm font-medium mb-2 text-right">
            تصاویر آگهی <span className="text-gray-400 text-xs">(حداکثر 10 تصویر)</span>
          </label>
          <div className="border-dashed border-2 border-gray-700 rounded-lg text-center hover:border-blue-500 transition-all duration-300 bg-gray-800/50 overflow-hidden group">
            <input
              ...
              id="imageUpload"
            />
            <label htmlFor="imageUpload" className="cursor-pointer block w-full h-full p-8 group-hover:bg-gray-800/80 transition-all duration-300">
              <div className="flex flex-col items-center justify-center text-gray-400 transform group-hover:scale-105 transition-transform duration-300">
                ...
                <span className="text-sm mb-2 font-medium">برای آپلود تصاویر کلیک کنید یا فایل‌ها را اینجا رها کنید</span>
                <span className="text-xs text-gray-500 bg-gray-800/70 px-3 py-1 rounded-full">فرمت‌های مجاز: JPG، PNG، WEBP</span>
              </div>
            </label>
          </div>
          <ErrorMessage name="images" component="div" className="text-red-500 text-sm mt-1 text-right" />
          
          {/* Image previews */}
          {previewImages.length > 0 && (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-4">
              {previewImages.map((src, index) => (
                <div key={index} className="relative group h-28 bg-gray-800 rounded-lg overflow-hidden border border-gray-700 shadow-md hover:shadow-lg transition-all duration-300 hover:border-blue-500">
        ...
        <div className="mb-6">
          <label className="block text-gray-200 text-sm font-medium mb-2 text-right">
            ویدیو <span className="text-gray-400 text-xs">(حداکثر 1 ویدیو)</span>
          </label>
          <div className="border-dashed border-2 border-gray-700 rounded-lg text-center hover:border-blue-500 transition-all duration-300 bg-gray-800/50 overflow-hidden group">
            <input
              type="file"
              ...
              id="videoUpload"
            />
            <label htmlFor="videoUpload" className="cursor-pointer block w-full h-full p-8 group-hover:bg-gray-800/80 transition-all duration-300">
              <div className="flex flex-col items-center justify-center text-gray-400 transform group-hover:scale-105 transition-transform duration-300">
                <VideoCameraIcon className="w-16 h-16 mb-4 text-purple-500 group-hover:text-purple-400" />
                <span className="text-sm mb-2 font-medium">برای آپلود ویدیو کلیک کنید یا فایل را اینجا رها کنید</span>
                <span className="text-xs text-gray-500 bg-gray-800/70 px-3 py-1 rounded-full">فرمت‌های مجاز: MP4، WEBM (حداکثر 50MB)</span>
              </div>
            </label>
          </div>
          <ErrorMessage name="videos" component="div" className="text-red-500 text-sm mt-1 text-right" />
          
          {/* Video previews */}
          {previewVideos.length > 0 && (
            <div className="mt-4">
              {previewVideos.map((src, index) => (
                <div key={index} className="relative group bg-gray-800 rounded-lg overflow-hidden border border-gray-700 shadow-md hover:shadow-lg transition-all duration-300 hover:border-purple-500">
                  <video 
                    src={src} 
                    controls 
      ...
      {category.formFields && category.formFields.length > 0 && (
        <div className="mb-8 border-t border-gray-800 pt-6">
          <h3 className="text-lg font-medium mb-4 text-right text-gray-300">اطلاعات تکمیلی</h3>
      ...
      <div className="mb-8 border-t border-gray-800 pt-6">
        <h3 className="text-lg font-medium mb-4 text-right text-gray-300">اطلاعات اصلی آگهی</h3>
        
        {/* Title */}
        <div className="mb-5">
          <label className="block text-gray-200 text-sm font-medium mb-2 text-right" htmlFor="title">
          ...
          />
          <ErrorMessage name="title" component="div" className="text-red-500 text-sm mt-1 text-right" />
        </div>

        {/* Description */}
        <div className="mb-6">
          <label className="block text-gray-200 text-sm font-medium mb-2 text-right" htmlFor="description">
          ...
          />
          <ErrorMessage name="description" component="div" className="text-red-500 text-sm mt-1 text-right" />
        </div>
      </div>
